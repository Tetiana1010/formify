const Item = ({productName, quantity, rate, description, handleItemChange, deleteItem}) => {

  return (
    <>
      <tr>
        <td className="p-1">
          <input
            name="productName"
            placeholder="Name"
            value={productName}
            onChange={handleItemChange}
            required
          />
        </td>
        <td className="p-1">
          <input
            name="quantity"
            placeholder="1"
            type="number"
            min={1} 
            step="any" 
            autoComplete="off" 
            value={quantity}
            onChange={handleItemChange}
            required
          />
        </td>
        <td className="p-1">
          <input
            name="rate"
            type="number"
            placeholder="0"
            min={0}
            step="any"
            autoComplete="off"
            inputMode="numeric"
            value={rate}
            onChange={handleItemChange}
            required
          />
        </td>
        <td className="p-1 text-end">
          $ {rate *quantity}
        </td>
      </tr>
      <tr>
        <td className="p-1" colSpan={3}>
            <textarea
              name="description"
              placeholder="Description of service or product..."
              value={description}
              onChange={handleItemChange}
              className="m-0 h-10"
              required
            />
        </td>
        <td className="flex gap-2 justify-end items-center text-blue-600 h-10">
          <button className="hover:text-blue-800">
            <svg className="h-6" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" d="m4.5 12.75 6 6 9-13.5" />
            </svg>
          </button>
          <button className="hover:text-blue-800" onClick={deleteItem}>
            <svg className="h-5" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" d="m14.74 9-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 0 1-2.244 2.077H8.084a2.25 2.25 0 0 1-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 0 0-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 0 1 3.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 0 0-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 0 0-7.5 0" />
            </svg>
          </button>
        </td>
      </tr>
    </>
  )
};

export default Item;